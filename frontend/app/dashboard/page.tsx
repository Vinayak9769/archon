...
const statusMap: Record<string, { label: string; color: string; bg: string; Icon: React.ElementType }> = {
  ...
  building_architecture:           { label: "Generating",      color: "text-zinc-400",  bg: "bg-zinc-800/10 border-zinc-700/20",  Icon: Loader2 },
  building_cpm:                    { label: "Generating",      color: "text-zinc-400",  bg: "bg-zinc-800/10 border-zinc-700/20",  Icon: Loader2 },
  building_database:               { label: "Generating",      color: "text-zinc-400",  bg: "bg-zinc-800/10 border-zinc-700/20",  Icon: Loader2 },
  validating:                      { label: "Validating PRD",  color: "text-zinc-400",    bg: "bg-zinc-800/10 border-zinc-700/20",      Icon: Loader2 },
  clarifying:                      { label: "Clarifying",      color: "text-zinc-400",  bg: "bg-zinc-800/10 border-zinc-700/20",  Icon: Clock },
};
...
const quickActions = [
  { href: "/projects/new", icon: Plus,        label: "New Design",   desc: "Start from a PRD",         color: "text-zinc-200 bg-zinc-850/40 border-zinc-700/30" },
  { href: "/projects",     icon: FolderGit2,  label: "All Projects", desc: "Browse your projects",     color: "text-zinc-300 bg-zinc-850/40 border-zinc-700/30" },
  { href: "/templates",    icon: FileText,    label: "Templates",    desc: "Start from a template",    color: "text-zinc-300 bg-zinc-850/40 border-zinc-700/30" },
  { href: "/reports",      icon: ArrowUpRight,label: "Reports",      desc: "Export design artifacts",  color: "text-zinc-200 bg-zinc-850/40 border-zinc-700/30" },
];
    ...
    <div className="p-6 max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link href="/projects/new"
          className="flex items-center gap-1.5 px-4 py-2 bg-zinc-100 hover:bg-zinc-200 text-zinc-950 text-xs font-semibold rounded-lg transition-all shadow-sm">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: "Projects",        value: loading ? "—" : String(projects.length), sub: "Total projects",       color: "text-zinc-250" },
          { label: "Active Designs",  value: loading ? "—" : String(inProgress),      sub: "In progress",          color: "text-zinc-200" },
          { label: "Awaiting Review", value: loading ? "—" : String(awaiting),        sub: "Your action needed",   color: "text-zinc-400" },
          { label: "Completed",       value: loading ? "—" : String(completed),       sub: "Designs finished",     color: "text-zinc-300" },
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Card className="bg-[#111113] border-zinc-800/60 col-span-2">
          <div className="p-4 border-b border-zinc-800/60 flex items-center justify-between">
            ...
            <Link href="/projects" className="text-xs text-zinc-400 hover:text-zinc-300 flex items-center gap-1">
          ...
          {!loading && recentDesigns.length === 0 && (
            <div className="flex flex-col items-center py-10 text-center">
              ...
              <Link href="/projects/new" className="text-xs text-zinc-400 mt-1 hover:text-zinc-300">Start your first →</Link>
            </div>
          )}
                ...
                <Link key={d.id} href={`/projects/${d.project_id}/design/${d.id}`}
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <Badge className={cn("text-[10px] border px-1.5 gap-1", cfg.bg, cfg.color)}>
                      <cfg.Icon className={cn("w-3 h-3", d.status.startsWith("building") && "animate-spin")} />
                      {cfg.label}
                    </Badge>
